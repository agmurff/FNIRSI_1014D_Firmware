...
**F29 — a disabled channel shows a garbage measurement (−22 V) (bench, 2026-07-12; fix not yet implemented).** `fpga_read_sample_data` is correctly *skipped* for a disabled channel (`scope_functions.c:882` enable guard), so `channel2.average` keeps a stale/uninit value; but `ui_display_measurements`/`ui_update_measurements` (`menu_1014d.c:1472/1513`) draw every slot **unconditionally** — no `settings->enable` guard. Fix: blank/dash the disabled channel's measurement slots.
